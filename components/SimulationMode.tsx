...
interface SimulationModeProps {
  audioFile?: File; // 使其可选，对于电话推流模式此参数为空
  activeCallId?: string | null; // 电话呼叫ID
  wsClient?: WebSocketClient | null; // WebSocket客户端
  provider: SpeechRecognitionProvider;
  onFinish: (notes: string, structured: StructuredDiagnosis, product: Product, script: RecommendationScript) => void;
  onClose: () => void;
}

const SimulationMode: React.FC<SimulationModeProps> = ({ audioFile, activeCallId, wsClient, provider, onFinish, onClose }) => {
  ...
  const startLiveProcessing = async () => {
    ...
    try {
      ...
      if (provider === 'volcano') {
        // 使用火山引擎服务
        const { appKey, accessKey, apiUrl: url, useProxy: useProxyConfig } = config.volcano;
        const volcanoService = new VolcanoEngineService(appKey, accessKey, url, useProxyConfig);
        volcanoServiceRef.current = volcanoService;

        setStatus('streaming');

        if (audioFile) {
          // 创建音频元素并播放音频文件（仅文件处理模式）
          ...
          setStatus('completed');
          triggerDeepAnalysis(true);
        } else if (wsClient && activeCallId) {
          // 电话推流模式：通过 WebSocket 接收音频数据
          console.log('📡 进入电话推流模式，等待音频数据...');

          let lastDataTime = Date.now();
          const streamSession = await volcanoService.startStreamingSession(
            (transcripts) => {
              handleVolcanoTranscript(transcripts);
              setCurrentTime((Date.now() - lastDataTime) / 1000); // 粗略估算当前时间
            },
            (error) => {
              console.error('Volcano streaming error:', error);
              setStatus('completed');
              isProcessingRef.current = false;
            }
          );
          ...
          // 监听推流数据
          const handleAudioData = (message: any) => {
            if (message.callId === activeCallId && message.audioData) {
              const audioBytes = Uint8Array.from(atob(message.audioData), c => c.charCodeAt(0));
              streamSession.sendAudio(audioBytes);
            }
          };
          ...
          throw new Error('未提供音频文件且未建立通话连接');
        }
    ...
    try {
      ...
      if (response.recommendedProductId && response.recommendedProductId !== 'none' && response.recommendedProductId !== lastAnalysis.productId) {
        ...
        if (product) {
          ...
          // 记录：推荐产品（需要从外部传入 doctor_id 和 call_id）
          // 注意：SimulationMode 组件目前没有这些信息，需要从 props 传入
          // 暂时先不记录，等后续支持推流模式时再添加
        } else {
          ...
          <div className="flex-1 overflow-y-auto space-y-6 custom-scrollbar pr-2 mb-6">
            ...
            {transcription.map((t, i) => (
              <div key={i} className={`animate-in slide-in-from-bottom-2 duration-500 flex flex-col ${t.role === 'Doctor' ? 'items-end text-right' : 'items-start text-left'}`}>
                ...
                <p className={`p-5 rounded-3xl text-[13px] leading-relaxed border shadow-xl max-w-[95%] ${t.role === 'Doctor'
